let vid = document.getElementById('girlTransition');
...
function toPrevious() {
  ...
  let x = window.event.clientX;
  if (x < w*0.15) {
    window.location.href = ("game-secondchoice.html");
  } else if (x > w*0.85) {
    window.location.href = ("girl_option.html");
  } else if (vid.paused) {
    document.body.style.cursor = "url('images/play.png'), auto";
    vid.play();
  } else if (vid.paused == false) {
...
function playPop() {
	...
	audio.play();
	setTimeout (function() {
  	window.open( "girl_option.html", "_self", "");
	}, audio.duration * 1000);
}
